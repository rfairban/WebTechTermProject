var makeRandom = function() {
  var combinations = this.getAttribute("combinationsArray");
  var combinationsArray = combinations.split(' ').map(Number);
  /*for(int i = 0; i < combinationsArray.length; i++) {
      if(combinationsArray[i] == "NaN") {
          alert("Error: Non-number detected within combinations list.")
      }
  } */ 
  ...
  for(i = 0; i < combinationsArray.length; i++) {
      for(x = 0; x < combinationsArray[i]; x++) {
          diceRollArray.push(i+min);
      }
  }
  ...
  if(!l1[1] && !l2[1])
    ...
    randomVal = diceRollArray[Math.floor(Math.random()*diceRollArray.length)].toFixed(decimalPlaces);
  }
